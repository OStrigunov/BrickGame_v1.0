...
![Конечный автомат тетриса](images/diagram_FSM.png)
...
- Обеспечено покрытие библиотеки unit-тестами с помощью библиотеки `check` и покрытие тестами составляет не меньше 80 процентов.
...
- Для управления реализована поддержка всех кнопок, предусмотренных на физической консоли:
  - Начало игры,
  - Пауза,
  - Завершение игры,
...
### Подсчет очков и рекорд в игре. Механика уровней
...
- make install - Сборка и установка игры в папку build
- make unistall - Удаление программы и очистка содержимого папки build
- make play - Запуск игры
- make test - Покрытие unit-тестами c помощью библиотеки Check
- make gcov_report - Формирование отчёта gcov в виде html страницы
- make dvi - Компиляция doxygen-файла с документацией
- make dist - Архивирование проекта
- make clang - Проверка кода с помощью clang-format
- make cppcheck - Проверка кода с помощью cpp-check
- make leaks - Проверка тестов на утечки памяти
- make clean - Удаление ненужных файлов

### Геймплей

![Геймплей](images/tetris.gif)
